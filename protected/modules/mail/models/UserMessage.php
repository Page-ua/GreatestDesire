...
class UserMessage extends ActiveRecord {
	...
	public static function getMessageTwoUsser( $curentUser, $anotherUser )
	{
		...
		$queryWeMessage = self::find();
		$queryWeMessage->select('message_id');
		$queryWeMessage->where(['in', 'message_id', $myMessage]);
		$queryWeMessage->andWhere(['user_id' => $anotherUser]);
		$weMessage = $queryWeMessage->one();
		!(empty($weMessage->message_id)) ? $result = $weMessage->message_id : $result = '';
		return $result;
	}
